...
- _lightengine.malloc(size): FreeRTOSヒープからメモリを確保し、Pythonの ytearray (参照渡し) として返す。
...
game/main.py で ytearray(12 * 1024) として確保している画像バッファを、上記で作成した _lightengine.malloc を用いるように変更します。
