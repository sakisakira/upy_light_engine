...
## ハマりポイント5: ESP-IDFのバージョン不整合によるビルドエラーとサブモジュールの罠
MicroPythonのカスタムCモジュールをビルドする際、ESP32向けのビルドイメージ（Docker）にインストールされている `ESP-IDF` のバージョンと、MicroPythonのリポジトリが要求するバージョンの間に微細な不整合があり、以下の2つのファイルでコンパイル/リンカエラーが発生しました。

1. `micropython/ports/esp32/network_wlan.c`: `WIFI_AUTH_MAX` のStatic Assertエラー。
2. `micropython/ports/esp32/panichandler.c`: `esp_panic_handler_reconfigure_wdts` の未定義（Undefined Reference）エラー。

* **罠の連鎖**: 一度これらの該当行をコメントアウトしてビルドを通したものの、後日「サブモジュールがdirty状態になっている」と勘違いして `git clean -fd` と `git checkout .` を実行した結果、**自ら当てたパッチを消し去ってしまい再びビルドエラーに陥る**という罠にハマりました。
* **解決策**: サブモジュールをクリーンアップしてもすぐに復旧できるように、この修正をGitパッチファイル（`patches/micropython_esp32_build_fixes.patch`）として切り出し、`apply_patches.ps1` スクリプトでいつでもパッチを当て直せるCI/CDフレンドリーな構成に変更しました。

## ハマりポイント6: カスタムファームウェアビルド時のPSRAM無効化の罠
Cモジュールを組み込んだカスタムファームウェアをビルドし、いざゲームのメインループ（`main.py`）を起動したところ、`engine/hal/framebuffer_micropython.py` で画面バッファ用メモリ（64800バイト）を確保しようとした瞬間に `MemoryError: memory allocation failed` でクラッシュする事態が発生しました。

* **罠の真相**: 公式ファームウェアやM5Stackの提供ファームウェアでは、Cardputer Advに搭載されている **8MBのOctal PSRAM** がデフォルトで有効化されています。しかし、自前で `BOARD=ESP32_GENERIC_S3` としてビルドした場合、デフォルト設定ではPSRAMが無効となり、ESP32-S3の極小な内蔵RAM（約300KB）しかMicroPythonのヒープとして割り当てられません。このカツカツのRAMの中でCモジュールのDMAバッファやFreeRTOSタスクが展開されたため、残りの連続領域が枯渇して64KBの画面バッファすら確保できなくなっていました。
* **解決策**: ビルドコマンド（`scripts/build_c_module.ps1`）のオプションに `BOARD_VARIANT=SPIRAM_OCT` を明示的に付与してリビルドすることで、8MBの超大容量ヒープが使えるようになり、メモリ不足問題は完全に解消しました。
...
* サブモジュール内のソースコードを直接書き換えて回避したエラーは、必ずPatchファイルとしてリポジトリ側で管理し、「いつ `git clean` されても復旧できる状態」を維持する。
* **「公式ファームウェアでは動いていたのに自作ファームウェアだとMemoryErrorで落ちる」時は、真っ先にPSRAM（SPIRAM）の有効化設定が漏れていないかを疑う。**
