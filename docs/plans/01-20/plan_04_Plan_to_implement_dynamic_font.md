...
None. The requirements are clear and can be elegantly implemented inside the engine without breaking existing APIs.
...
- **Dynamic Color**:
  ...
  - Inside the `is_argb` block, add logic to check `if color != -1:`. If true, extract the source RGB compon
