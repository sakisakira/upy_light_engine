...
def _exit_app():
    from engine import sound, logger
    if hasattr(sound._hal, 'stop'):
        sound._hal.stop()
    global _root
    if _root:
        _root.destroy()
...
def _handle_interrupt_signal(sig=None, frame=None):
    # TODO(Plan 42 stub):
    # Future enhancement: Instead of immediate exit, route this signal to the game 
    # to open a pause menu or trigger a specific state change.
    _exit_app()
...
def init(width, height, title="Pyxel (CPython + Native C Engine)", fps=30, scale=2):
    ...
    _root.protocol("WM_DELETE_WINDOW", _exit_app)
