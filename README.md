...
### How to build assets
You must convert standard images and fonts into the engine's highly optimized proprietary formats.
1. **`.uimg` (Micro Image)**
   - Custom 8-bit indexed image format (INDEX8).
   - Generated by converting a directory of PNGs using the `convert_assets.py` tool.
   ```bash
   python tools/convert_assets.py assets/images/ assets/images/palette.bin
   ```
